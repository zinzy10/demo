...
class dashData {
	...
	function selectYears() {
		...
		$select = $this->obj->consultar("SELECT fecha, YEAR(fecha) AS years, SUM(sale) AS total FROM sales GROUP BY years ORDER BY years DESC");
	...
	function selectClient() {
		...
		$client = $this->obj->consultar("SELECT client FROM clients GROUP BY client");
	...
	function tableClient() {
		...
		$sql = "SELECT
				...
				FROM sales
				INNER JOIN clients ON sales.id_clients = clients.id_client
				WHERE YEAR(fecha) IN ('2022', '2023', '2024', '2025')
				GROUP BY client";
