...
class dashData {
    ...
    public function __construct() {
        $this->data = parse_ini_file(__DIR__ . "/../../../env/.env");
    ...
    function packingF1() {
        ...
        $query = "SELECT
                    ...
                    FROM {$this->BD}production.packing
                    WHERE estado != 'DELETE'
                    AND machine LIKE '%MP1%'
                    $time
                    $shift
                    GROUP BY machine WITH ROLLUP";

        $PF1 = $this->obj->consultar($query, $params);
        ...
        if (empty($PF1)) {
            ...
            foreach ($PF1 AS $pack) {
                $press = $pack['machine'];
                $firstQuality = number_format($pack['total_quality']);
                $totalMill = number_format($pack['total_mill']);
                $totalDamage = number_format($pack['total_damage']);
                $totals = number_format($pack['totals']);
    ...
    function packingF7() {
        ...
        $query = "SELECT
                    ...
                    FROM {$this->BD}production.packing
                    WHERE estado != 'DELETE'
                    AND machine LIKE '%MP7%'
                    $time
                    $shift
                    GROUP BY machine WITH ROLLUP";

        $PF7 = $this->obj->consultar($query, $params);
    ...
    function totalPack() {
        ...
        $query = "SELECT
                ...
                WHERE (machine LIKE '%MP7%' OR machine LIKE '%MP1%')
                $time
                AND estado != 'DELETE'
                $shift";

        $PF1_7 = $this->obj->consultar($query, $params);

        if (empty($PF1_7)) {
            return json_encode(["data"=>[]]);
        } else {
            foreach ($PF1_7 AS $pack) {
                $firstQuality = number_format($pack['total_quality']);
                $totalMill = number_format($pack['total_mill']);
                $totalDamage = number_format($pack['total_damage']);
                $totals = number_format($pack['grand_total']);
